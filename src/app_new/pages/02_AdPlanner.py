...
DEFAULT_K    = 12
CONTEXT_N    = 300                # fixed number of grey context candidates
...
always-on list by averaging scores across the selected hours.
...
def load_candidates() -> pd.DataFrame:
    """
    Citește toate fișierele 'candidates_*.parquet' din folderul de Google Drive
    setat în st.secrets['drive']['folder_url'] (vezi data_loader.py).
    """
    # listăm relativ la rădăcina folderului Drive (cached de data_loader)
    drive_files = list_drive_files()

    # Acceptă fie 'ads/candidates_123.parquet', fie direct 'candidates_123.parquet'
    pattern = re.compile(r"(?:^|/)(?:ads/)?candidates_\d+\.parquet$", re.IGNORECASE)
    paths = [p for p in drive_files if pattern.search(p)]
    if not paths:
        # fallback generos dacă numele diferă ușor
        paths = [p for p in drive_files
                 if p.lower().endswith(".parquet") and "candidates" in os.path.basename(p).lower()]

    if not paths:
        st.error("No candidate files found in Drive folder (e.g., 'ads/candidates_0.parquet').")
        st.stop()

    # Citește din Drive (se descarcă în cache local o singură dată)
    dfs = [load_parquet_from_folder(p) for p in sorted(paths)]
...
def available_score_options(df: pd.DataFrame):
    """What each score means (shown again in the sidebar)."""
    opts, colmap, desc = [], {}, {}
...
def norm01(s: pd.Series) -> pd.Series:
    ...
    if hi <= lo: return pd.Series(np.zeros(len(s)), index=s.index)
    return np.clip((s - lo) / (hi - lo), 0, 1)

def initial_view_points(df: pd.DataFrame) -> tuple[float, float, int]:
    if len(df) == 0: return (*DEFAULT_VIEW, 10)
    lat = df["lat"].astype(float).to_numpy(); lon = df["lon"].astype(float).to_numpy()
    lat_c, lon_c = float(np.nanmean(lat)), float(np.nanmean(lon))
...
def add_boundary(layers: list, filename="h3_frontier.geojson", rgba=[0,0,0,150], width=1):
...
# robust exporter: drop helper cols and serialise scalars/lists safely
...
# colour mapping (cividis); returns RGBA 0–255
...
def hour_colour_legend():
    """Small legend with ticks at 0, 6, 12, 18, 23."""
...
hours_available = sorted(df_all["hour"].dropna().unique().astype(int).tolist())

# ---------- Sidebar (with inline explanations) ----------
...
if scol not in df_scope.columns:
    ...
    st.stop()
df_scope["score_used"] = df_scope[scol].astype(float)

if scope == "Single hour":
    df = df_scope.copy()
else:
    # aggregation: mean across selected hours
...
if scope == "All hours (aggregate)":
    ...
    idx = peak.groupby(keys)["score_used"].idxmax()
    ...
    selected["hour_label"] = selected["hour_peak"].astype("Int64")  # shown in tooltip/table
    ...
    selected["_color"] = [[255, 120, 20, 220]] * len(selected)  # constant orange
    selected["score_disp"] = selected["score_used"].round(2)
...
# ---------- Map ----------
layers = []
if show_boundary: add_boundary(layers)
...
# selected points (size by score, colour by hour if aggregate)
layers.append(pdk.Layer(
    "ScatterplotLayer",
...
# one-line reminder under the map
if scope == "All hours (aggregate)":
...
- **score_peak** — (*All hours only*) the best score for this location among the included hours (useful to gauge daily variability).  
- **h3_cell** — H3 index of the cell where the panel sits (resolution as in the pipeline).  
- **lat / lon** — panel anchor in WGS84 (degrees).
"""
)

# ---------- Export (drop helper columns starting with '_') ----------
...
*Dwell-focused* (higher where dwell is longer), or *KDE proxy* (density of movement through the hex).
